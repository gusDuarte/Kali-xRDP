Write-Output [%TIME:~0,8%] Check if WSL is enabled, if not, enable and RESTART !!!.
...
Set-Variable RUNSTART=%date% @ %time:~0,5%

Write-Output
Write-Output [%TIME:~0,8%] Check if Virtual Machine feature is enabled, it not, enable and RESTART !!!
...
Write-Output [%TIME:~0,8%] Download and Install Linux Kernel Update
...
Write-Output [%TIME:~0,8%] Set WSL version 2.
