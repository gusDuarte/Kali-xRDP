...
PowerShell.exe -Command "IF (Test-Path $env:TEMP\vmenable.TMP) {$do = Get-Content $env:TEMP\vmenable.TMP}; IF ($do -ne 'done') {dism.exe /online /enable-feature /featurename:VirtualMachinePlatform /all /norestart}; ECHO 'done' > $env:TEMP\vmenable.TMP"
ECHO:
ECHO [%TIME:~0,8%] Download and Install Linux Kernel Update
PowerShell.exe -Command "IF (Test-Path $env:TEMP\kernelupdate.TMP) {$do = Get-Content $env:TEMP\kernelupdate.TMP}; IF ($do -ne 'done') {Start-BitsTransfer -Source https://wslstorestorage.blob.core.windows.net/wslblob/wsl_update_x64.msi -Destination $env:TEMP\wsl_update_x64.msi ; $env:TEMP\wsl_update_x64.msi; ECHO 'done' > $env:TEMP\kernelupdate.TMP}"
